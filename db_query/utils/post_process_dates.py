...
class PostProcessDatesClass:
    ...
    def only_year(self,date):
        ...
        if int(int_year):
            first_day_year = parser.parse(int_year).replace(month=1).replace(day=1).strftime('%d-%m-%y')
            last_day_year = parser.parse(int_year).replace(month=12).replace(day=31).strftime('%d-%m-%y')
    ...
    def only_month(self,date):
        ...
        if date in months:
            ...
            first_day = date.replace(day=1).strftime('%m/%d/%y')
            last_day = date.replace(day = calendar.monthrange(date.year, date.month)[1]).strftime('%d-%m-%y')
    ...
    def quarter(self,date):
        ...
        if date in quarter:
            ...
            if '1' in date:
                first_date = temp_date.replace(month=1).replace(day=1).strftime('%d-%m-%y')
                last_date = temp_date.replace(month=3).replace(day=31).strftime('%d-%m-%y') 
            if '2' in date:
                first_date = temp_date.replace(month=4).replace(day=1).strftime('%d-%m-%y')
                last_date = temp_date.replace(month=6).replace(day=30).strftime('%d-%m-%y')
            if '3' in date:
                first_date = temp_date.replace(month=7).replace(day=1).strftime('%d-%m-%y')
                last_date = temp_date.replace(month=9).replace(day=30).strftime('%d-%m-%y')
            if '4' in date:
                first_date = temp_date.replace(month=10).replace(day=1).strftime('%d-%m-%y')
                last_date = temp_date.replace(month=12).replace(day=31).strftime('%d-%m-%y')
    ...
    def this_month(self):
        ...
        """
        date = datetime.now()
        first_day = date.replace(day=1).replace(hour=0,minute=0,second=0).strftime('%d-%m-%y')
        last_day = date.replace(day = calendar.monthrange(date.year, date.month)[1]).replace(hour=11,minute=59,second=59).strftime('%m/%d/%y %H:%M:%S')
        dates = []
        dates.append(first_day)
        dates.append(last_day)
        return dates
    ...
    def previous_month(self):
        ...
        first_day = last_month.replace(day=1).replace(hour=0,minute=0,second=0).strftime('%d-m-%y')
        last_day = last_month.replace(day = calendar.monthrange(last_month.year, last_month.month)[1]).replace(hour=11,minute=59,second=59).strftime('%m/%d/%y %H:%M:%S')
    ...
    def last_week(self):
        ...
        dates = []
        dates.append(first_day.replace(hour=0,minute=0,second=0).strftime('%d-%m-%y'))
        dates.append(last_day.replace(hour=11,minute=59,second=59).strftime('%d-%m-%y'))
        return dates
    ...
    def post_process(self,date):
        ...
        if date not in ['last week','last month','this week','this month','previous month']:
            date = self.text2int(date).strip()
        else:
            date = date.strip()
        ...
        if 'today' in date:
            final_date = []
            date = datetime.now()
            date_start = date.replace(hour=0,minute=0,second=0).strftime('%d-%m-%y')
            date_end = date.replace(hour=11,minute=59,second=59).strftime('%d-%m-%y')
        ...
        elif 'this month' in date:
            final_date = self.this_month()
        elif 'this week' in date:
            final_date = self.this_week()
        else:
            final_date = []
            final_date_start = parser.parse(date).replace(hour=0,minute=0,second=0).strftime('%d-%m-%y')
            final_date_end = parser.parse(date).replace(hour=11,minute=59,second=59).strftime('%d-%m-%y')
